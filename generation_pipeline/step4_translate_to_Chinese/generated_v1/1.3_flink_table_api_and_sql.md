...
## 何时使用 Table API vs DataStream API
...
**说明**:
- `executeSql().print()` 可以处理来自聚合的变更流（INSERT/UPDATE/DELETE）
- `op` 列是 Flink 自动添加的，用于标识操作类型：
  - `+I` = INSERT（新记录）
  - `-U` = UPDATE_BEFORE（更新前的旧值）
  - `+U` = UPDATE_AFTER（更新后的新值）
  - `-D` = DELETE（删除的记录）
- 在本例中，当单词再次出现时，Flink 会发送 `-U`（旧计数）和 `+U`（新计数）来更新聚合结果
- 在流处理模式下，不支持对非时间属性进行 ORDER BY
- 结果按到达顺序打印（不保证顺序）
