...
将 checkpoint 测试想象成测试备份系统：

- **Checkpoint Creation** = 备份创建："备份是否被创建？"
- **Simulating Failures** = 模拟灾难："如果系统崩溃怎么办？"
- **State Recovery** = 从备份恢复："我们能否正确恢复所有内容？"

**Why Needed**: MarketLag 需要确保容错性有效 - 如果作业失败，它可以从 checkpoints 恢复而不会丢失数据。

---

## Analogy

如果您了解 **database backup testing**（您了解），checkpoint 测试是类似的：
- Database backup = Flink checkpoint（两者都创建快照）
- Backup restore = Checkpoint recovery（两者都从快照恢复）
- Backup testing = Checkpoint testing（两者都验证备份/恢复是否有效）

关键区别：Flink checkpoints 是连续和自动的。

---

## Relationship to Already Learned Topics

- **1.10 Checkpoints**: 理解 checkpoint 机制
- **10.1 Local Testing**: Checkpoint 测试使用本地环境
- **Fault Tolerance**: 您已经了解容错性 - 这测试它
