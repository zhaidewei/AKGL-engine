...
public class TimeStampDemo {
    public static void main(String[] args) throws Exception {
        ...
        // Create events with ISO 8601 timestamps (supports both "Z" and "+08:00" formats)
        @SuppressWarnings("deprecation")
        DataStream<Event> events = env.fromElements(
            new Event("event1", "login", "2024-01-01T20:00:00+08:00"),  // Beijing time (UTC+8)
        ...
        // Align to UTC hour boundary (round down to the start of the hour)
        DataStream<Event> aligned = withTimestamps.map(event -> {
            // Convert timestamp to UTC ZonedDateTime, then align to hour boundary
            ZonedDateTime zonedDateTime = Instant.ofEpochMilli(event.getTimestamp())
                .atZone(ZoneId.of("UTC"))
                .withMinute(0)
                .withSecond(0)
                .withNano(0);
            // Set the aligned timestamp (start of the hour)
...
class Event {
    ...
    public Long alignedTimestamp;  // Timestamp aligned to hour boundary
    ...
    public Event(String user, String action, String timestamp) {
        this.user = user;
        this.action = action;
        // Parse ISO 8601 timestamp (supports both "Z" and "+08:00" formats)
        try {
            // Try Instant.parse first (for "Z" format)
            this.timestamp = Instant.parse(timestamp).toEpochMilli();
        } catch (Exception e) {
            // If that fails, try ZonedDateTime.parse (for "+08:00" format)
