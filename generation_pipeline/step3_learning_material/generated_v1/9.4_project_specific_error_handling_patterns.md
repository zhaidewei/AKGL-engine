...
Think of project-specific error handling like custom safety rules:

- **Missing RSS Data** = No news: "No RSS events this hour, skip window"
- **Missing Price Data** = No price: "Price missing, use default (delta = 0)"
- **API Rate Limits** = Too many requests: "Wait and retry, if still fails send to DLQ"
- **Schema Evolution** = Schema changes: "Handle old and new schema versions"

**Why Needed**: MarketLag has specific error scenarios that need tailored handling.

---

## Analogy

If you know **error handling patterns** (which you do), project-specific patterns are similar:
- Generic error handling = Project-specific patterns (both handle errors)
- Error recovery = Custom recovery logic (both recover from errors)

Key difference: Project-specific patterns are tailored to MarketLag's unique scenarios.

---

## Relationship to Already Learned Topics

- **9.1 Dead Letter Queue**: DLQ used for API rate limit failures
- **9.2 Data Validation**: Validation handles missing data
- **9.3 Exception Handling**: Exception handling implements these patterns
- **3.3 Equi-Join**: Missing price data handled in joins
